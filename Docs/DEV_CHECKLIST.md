...
- **#219 K14.** Read↔Listen seamless playback continuity + cast-routing parity on /read — **SHIPPED 2026-05-06.** New `apps/web/src/lib/playback-store.ts` exposes canonical localStorage keys (`qalaam-reciter`, `qalaam-verse-key`, `qalaam-playing`) shared by MiniPlayer (/listen) AND ContinuousReaderPlayer (/read). Switching between pages now carries the user's reciter, verse, and playback state — no more "audio stops when I switch tabs". On /read, ContinuousReaderPlayer now wires the same `useCast` hook MiniPlayer uses: every active-bundle URL change pushes to the receiver via `loadMedia(autoplay=true)`, the chicken-and-egg routing-flag deadlock is broken (uses `cast.isConnected` alone), and `cast.onMediaEnded` drives `onEnded()` so cross-surah chaining works on the receiver too. Manual prev/next at surah boundaries now navigates to the adjacent surah (`/read/N±1?continue=1`) instead of disabling. Local audio plays muted (volume=0) during cast so the rAF-driven word-by-word highlight loop stays accurate against the receiver's playback.
