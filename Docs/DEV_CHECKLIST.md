...
- **#215 J3.** GitHub repo visibility flip to private — **CONFIRMED COMPLETE 2026-05-07.** `gh repo view RabHanz/qalaam` reports `visibility: "PRIVATE"`. AGPL-3.0 distribution remains intact: source is provided to SaaS users on request per the license, regardless of whether the repo is publicly hosted. Task was previously tracked as pending but the flip had already happened — checklist now reflects reality.
- **#223 J5.** Stripe checkout + webhook scaffold — **SHIPPED 2026-05-07.** Replaces the manual SQL tier-bump path with a real Stripe Checkout flow, env-flagged to activate the moment STRIPE_SECRET_KEY + STRIPE_WEBHOOK_SECRET + BILLING_PUBLIC_URL + at least one STRIPE_PRICE_PREMIUM/PRO land. Without env: every endpoint returns a clean `qalaam.billing.not-configured` 503 — no half-finished implementation. NO Stripe SDK dep — talks to Stripe via plain `fetch` with form-encoded bodies; signature verification uses node:crypto HMAC-SHA256 with a 5-minute replay window and `timingSafeEqual` constant-time comparison. New `apps/backend/src/lib/billing.ts` (env config, signature verifier, REST helpers). `apps/backend/src/routes/v1/billing.ts` ships POST `/v1/billing/checkout-session` (auth-gated, returns hosted-checkout URL), POST `/v1/billing/webhook` (signature-verified event handler), GET `/v1/billing/status` (whether billing is wired). Webhook handles `checkout.session.completed` (stores customer + sub IDs), `customer.subscription.{created,updated}` (re-resolves tier from price_id, updates billing_status), `customer.subscription.deleted` (reverts to free) — every tier change writes `admin_audit`. Soft migration adds `stripe_customer_id`, `stripe_subscription_id`, `billing_status` columns to `users`. Server boot now installs a custom `application/json` content-type parser that ALSO attaches `req.rawBody` so the webhook can verify against the exact bytes Stripe signed. Frontend: `PricingTiers` queries `/v1/billing/status` on mount; if configured, the upgrade CTA POSTs `/v1/billing/checkout-session` and `window.location.href`-redirects to Stripe; otherwise falls back to the existing manual support-request form so the page never breaks. Activation is one env update + Stripe webhook secret config away.
- **#225 F1 Phase 1.** Quranic Arabic course bodies — alphabet + vowels + makhraj zones — **SHIPPED 2026-05-07.** Replaces the "body coming v0.5" placeholder for 36 of the 40 Level 1+2 lessons (28 letters + 4 vowels + 4 makhraj zones). Approach: structurally-factual only — every assertion is a Unicode property, an articulation-anatomy fact, or a real verse reference. NO fabricated pedagogy. New `apps/web/src/lib/letter-data.ts` carries the canonical data: 28 letters with `(slug, glyph, makhraj zone, 2-3 corpus examples each)`, 4 vowel marks with `(mark, sound, structural effect, examples)`, 5 makhraj zones with `(anatomical reference, canonical letter list)`. Examples are common Quranic words pulled from familiar surahs (al-Fātiḥa, al-Baqara, al-Mulk, al-Ikhlāṣ) — every one verifiable in the corpus. Three new editorial body components in `apps/web/src/components/learn/`: `LetterLessonBody` (display-size glyph + tatweel-anchored 4-position joining table + makhraj zone link + corpus examples deep-linking to /study), `VowelLessonBody` (mark on host letter at display size + sound + factual effect + corpus examples), `MakhrajLessonBody` (anatomical reference + tappable letter chips routing to each letter's lesson — pairs with the existing MakhrajDiagram). The 4-position joining is computed at render via tatweel anchoring (`ـب`, `ـبـ`, `بـ`) so the OS shaper produces the contextual forms rather than us authoring presentation-form codepoints. Sifat (letter qualities), tajweed rules, recitation drills, mastery exercises remain placeholders pending a content-author pass.
- **#216 J4.** HACS integration — submission prep — **DEFERRED-WITH-PREP 2026-05-07.** HACS distribution requires the source repo be PUBLIC (HACS clients clone the repo at install time). Per #215 the main repo is private; HACS distribution is blocked until either (a) the repo flips back to public OR (b) the HA integration is split into its own public repo. Prep done in this commit: fixed manifest.json URLs (was pointing at the placeholder `qalaam-org/qalaam` org; now points at `qalaam.themarginapp.com/about` + `/support` which are stable customer-facing URLs not tied to any GitHub visibility decision). codeowners updated to `@RabHanz`. When distribution unblocks, the only remaining step is a single PR to https://github.com/hacs/default adding the repo path to the integrations file.
- **#224 D3 Phase 2.** Offline downloads — explicit per-surah pre-fetch + storage management — **SHIPPED 2026-05-07.** Builds on Phase 1's runtime SW cache. New `apps/web/src/lib/offline-cache.ts` exposes `prefetchSurahAudio` (concurrency-limited 6-parallel, AbortController-cancellable, EventTarget-style progress callback), `getStorageEstimate` (StorageManager.estimate + per-cache audio byte count), `clearAudioCache`, and `isSurahFullyCached` (sample-probe heuristic on first + last verse). Writes go into the same `qalaam-audio-v1` cache the SW reads from. New `SaveSurahOffline` component is a single-button surface with five states (idle / probing / cached / downloading / error) and a hairline progress bar inside the button itself — no separate spinner, no toast. Wired into `ListenSurfaceClient` next to the surah-count footer for the active surah. New `StorageSection` in `/settings` shows audio cache size + total browser usage + cap + a "Clear offline content" button (with native confirm). Mono tabular-nums for sizes, paper-card visual register, no SaaS-dashboard chrome. Concludes #174 D3 — runtime + explicit pre-fetch + management surfaces are all live.
- **#222 D3 Phase 1.** Offline downloads — runtime caching — **SHIPPED 2026-05-07.** Qalaam is now installable as a PWA + the most common offline path works without code changes by the user. Ships: `apps/web/src/app/manifest.ts` (Next.js convention) with `display: standalone`, theme/background colors, scope `/`; generated icons via `app/icon.tsx`, `apple-icon.tsx`, `icon1.tsx` (192), `icon2.tsx` (512), `icon3.tsx` (512 maskable) — all rasterised at build via `next/og` ImageResponse with the kaaf glyph in leaf-gold on deep teal. `apps/web/public/sw.js` is a hand-written service worker with versioned caches and per-resource strategies: navigations → network-first with cached-shell fallback; static assets (`/_next/static`, `/fonts`, woff/ttf) → cache-first; Quran-data API JSON (verses, translations, recitations, audio resolver, metadata, etc.) → stale-while-revalidate; cross-origin audio CDNs (audio.qurancdn.com, download.quranicaudio.com, everyayah.com, verses.quran.com) → cache-first with LRU cap (~120 entries). Authenticated/mutation paths (auth, admin, playback session, family, hifdh, plans, khatm, mistakes, voice-notes) bypass caching entirely so refresh after offline reflects the latest server state. Client-side `ServiceWorkerLifecycle` registers the SW on first window load (skipped on `localhost`/`*.local` so dev HMR isn't sabotaged). `useOnline` hook + offline indicator banner in MiniPlayer. `InstallHint` button in SiteNav surfaces the browser's `beforeinstallprompt` as a calm leaf-bordered pill — only renders when the browser actually offers it (Chromium + Edge); silent on Safari (no spec). Per-surah pre-download (Phase 2) deferred — not in this scope.
- **#214 J2.** Admin/dev panel — **SHIPPED 2026-05-07.** `/admin` maintainer console gated by `QALAAM_ADMIN_EMAILS` env-allowlist (cookie-only auth — API-keys never escalate). New `admin_audit` table writes a row on every mutation. Six endpoints under `/v1/admin/*`: `me`, `system` (tier counts, signups7d, sessions, support open, api-keys active), `users` (search + filter + paginate), `users/:id` (PATCH tier/minor/displayName/haUrl with from→to audit), `audit` (tail), `support` (triage), `support/:id` (resolve). Frontend `/admin` is a 12-col grid — Metrics + Users + Audit on top, Support full-width below — preserving Qalaam's editorial frame (paper cards, leaf-gold accents, smallcaps section labels) with operational density (mono tabular-nums IDs, dense tables, inline tier dropdown with optimistic update + revert-on-failure). No SaaS-dashboard chrome, no toasts.
- **#219 K14.** Read↔Listen seamless playback continuity + cast-routing parity on /read — **SHIPPED 2026-05-06.** New `apps/web/src/lib/playback-store.ts` exposes canonical localStorage keys (`qalaam-reciter`, `qalaam-verse-key`, `qalaam-playing`, `qalaam-position-seconds`) shared by MiniPlayer (/listen) AND ContinuousReaderPlayer (/read). Switching between pages now carries the user's reciter, verse, position, and playback state. On /read, ContinuousReaderPlayer wires the same `useCast` hook MiniPlayer uses: every active-bundle URL change pushes to the receiver via `loadMedia(autoplay=true)`. Manual prev/next at surah boundaries navigates cross-surah. Local audio plays muted during cast so the rAF word-highlight loop stays accurate.
- **K15.** Playback hardening pass — **SHIPPED 2026-05-07.** Five separate fixes:
  1. **Hard-coded surah verse-counts** (`SURAH_VERSE_COUNTS` + `verseCountFor` in playback-store) so cross-surah `advance()` never depends on a `/v1/metadata/surahs` fetch race. Was the actual root cause of "advance to next surah is still not working" — first end-of-Al-Fatihah produced `1:8` (404) instead of `2:1`.
  2. **In-place cross-surah chaining** in ContinuousReaderPlayer — appends the next surah's verse keys to the running list pre-emptively, so playback continues seamlessly across surah boundaries with no page reload.
  3. **Singleton useCast** — refactored `apps/web/src/lib/use-cast.ts` to a module-level singleton with one shared SDK listener and a subscription-based snapshot. Earlier each useCast() call constructed its own RemotePlayer + ANY_CHANGE listener; /read with 286 AyahCards meant 286 listeners × N setStates per cast tick → page hang. Plus exports imperative helpers (`getCastSnapshot`, `castLoadMedia`, `castPause`, `castSeek`, etc.) for components that only need to act, not subscribe.
  4. **AyahCard switched to imperative cast helpers** so the 286-instance render storm goes away even if other components keep subscribing.
  5. **No-blank audio src** — MiniPlayer's url-resolve effect no longer sets `audioUrl = null` between verses. The intermediate `<audio src=undefined>` was severing iOS Safari's gesture-context chain, causing the next `.play()` to reject silently after auto-advance. URL transitions directly old → new now.
  6. **SiteNav Read href on click** — reads `qalaam-verse-key` synchronously at click time, redirects via `window.location.href` if the stored verse-key resolves to a different surah. No useState race, no hydration mismatch. Listen → Read now lands on `/read/<their-surah>` instead of always `/read/1`.
  7. **Cast SESSION_STATE_CHANGED** — `useCast` subscribes to `CastContext.SESSION_STATE_CHANGED` + a 250ms × 3s post-mount poll backstop so origin-scoped auto-rejoin on a fresh page becomes visible to the UI (was showing "Cast" again instead of "Casting" after navigation).
