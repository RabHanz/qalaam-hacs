...
| 17 — QUL deep ingestion (v0.5 → v2.0)                | 22    | 22   | **100%** | License-aware framework + 7 sub-readers + ingest framework + 5 ingest scripts + inventory doc + ADR-0020 + 6 backend routes + UI consumption. **Real QUL data ingested via `scripts/data/bootstrap-qul-from-dump.sh`** (downloads + restores public mini Postgres dump from `static-cdn.tarteel.ai`, exports to `data/qul-source/*.json`, runs the ingest scripts). Live in `data/qul.sqlite`: 1,013 metadata rows (114 surahs + 30 juz + 60 hizbs + 240 rubs + 7 manzils + 558 rukus + 4 sajdas) + 22,220 word-by-word translations. Backend smoke-tested: `/v1/metadata/surahs/1`, `/v1/metadata/juz/30`, `/v1/wbw/1:1` return real data with attribution. Mutashabihat + similar-ayahs + recitation-segments + mushaf-layouts NOT in mini dump (pending production dump or scrape).                                                                                                                 |
