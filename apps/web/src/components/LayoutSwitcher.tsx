...
export function LayoutSwitcher({
  ...
  return (
    <div role="group" aria-label="Mushaf layout" className="inline-flex items-center gap-1 flex-wrap">
      <span className="smallcaps text-leaf text-xs mr-3">Layout</span>
      {availableLayouts.map((slug) => {
        ...
        return (
          <button
            ...
            className={`px-3 py-1.5 text-xs smallcaps transition-colors rounded-sm ${
              isCurrent
                ? 'bg-ink text-paper'
