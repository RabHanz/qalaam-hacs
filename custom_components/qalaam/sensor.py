...
- ``sensor.qalaam_current_verse``            — verse currently playing (text).
- ``sensor.qalaam_streak_days``              — Hifdh streak (TOTAL_INCREASING).
- ``sensor.qalaam_today_session_count``      — portions due today (MEASUREMENT).
- ``sensor.qalaam_grace_days_remaining``     — grace days left (MEASUREMENT).
- ``sensor.qalaam_current_sabqi``            — current sabqi range.
- ``sensor.qalaam_next_prayer``              — next salah ISO (TIMESTAMP).
- ``sensor.qalaam_topic_of_day``             — daily topic.
- ``sensor.qalaam_word_of_day``              — daily Arabic word.
- ``sensor.qalaam_hijri_date``               — formatted Hijri date.
- ``sensor.qalaam_mutashabihat_count``       — similar-ayah pairs in current portion.
- ``sensor.qalaam_active_reciter``           — currently playing reciter slug.
- ``sensor.qalaam_ramadan_phase``            — suhoor / day / iftar / taraweeh
                                               / odd_night / none. Drives the
                                               Ramadan-scenes blueprint.
- ``sensor.qalaam_family_khatm_juz_completed`` — juz completed by the family
...
from .coordinator import QalaamCoordinator, QalaamSnapshot
...
class QalaamSensor(QalaamEntity, SensorEntity):
    ...
    def native_value(self):  # type: ignore[no-untyped-def]  # noqa: PLR0911 PLR0912 ANN201 — straight-line dispatch over a tagged-union sensor key
        ...
        if key == "family_khatm_juz_completed":
            ...
            # `qalaam_family_khatm_milestone` rather than this sensor.
            return 0
...
def _compute_ramadan_phase(snap: QalaamSnapshot) -> str:  # noqa: PLR0911 — straight-line dispatch over a tagged-union phase
