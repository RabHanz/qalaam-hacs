"""Qalaam media_player entity.

Per ADR-0003: this entity is a *proxy* — it forwards play_media to a configured
target media_player (Cast, Sonos, AirPlay, etc.). Qalaam does not directly drive
...
Qalaam entity always reflects what's actually playing on the resolved speaker.
"""
...
async def async_setup_entry(
    ...
    async_add_entities([QalaamMediaPlayer(coordinator)])


class QalaamMediaPlayer(QalaamEntity, MediaPlayerEntity):
    """The Qalaam media_player. Routes play_media to a configured target."""
    ...
    _attr_translation_key = "qalaam_player"
    ...
    async def async_play_media(
        ...
        **kwargs: Any,
        ...
        If a media-source URI is passed (`media-source://qalaam/...`), HA's
        `media_source.async_resolve_media` resolves it before calling this.
        We just forward the resolved URL to the target.
        """
        if not self._target_entity_id:
            _LOGGER.warning(
                "qalaam: no target_player configured; set one in Options to enable forwarding."
            )
            return
