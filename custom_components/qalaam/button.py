"""Qalaam button entities — quick-action triggers wired to the live backend.

- ``button.qalaam_test_me``        → fetch today's Hifdh session and play the
                                     first item's verse on the configured
                                     target speaker. Also fires the
                                     ``qalaam_hifdh_session_started`` HA
                                     event for any existing automations
                                     still listening for it.
- ``button.qalaam_mark_memorized`` → record a Smooth rating against the
                                     user's most-recent portion (auto-creates
                                     an Al-Fatihah portion if none exists).
                                     Also fires ``qalaam_portion_marked_memorized``.
...
Press latency is dominated by the round-trip to the Qalaam backend
...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            QalaamTestMeButton(coordinator),
            QalaamMarkMemorizedButton(coordinator),
            QalaamRecordMistakeButton(coordinator),
            QalaamReplayLastButton(coordinator),
        ]
    )
...
class QalaamTestMeButton(QalaamEntity, ButtonEntity):
    ...
    def __init__(self, coordinator: QalaamCoordinator) -> None:
        ...
        self._attr_suggested_object_id = "qalaam_test_me"
    ...
    async def async_press(self) -> None:
        ...
        # listeners can branch on which verse Qalaam picked.
        ...
        if not target or not first_verse:
            ...
            _LOGGER.debug("qalaam.test_me: no playback (target=%s verse=%s)", target, first_verse)
            return
        ...
        try:
            ...
            _LOGGER.warning("qalaam.test_me: play_media failed for %s: %s", target, err)
...
class QalaamMarkMemorizedButton(QalaamEntity, ButtonEntity):
    ...
    def __init__(self, coordinator: QalaamCoordinator) -> None:
        ...
        self._attr_suggested_object_id = "qalaam_mark_memorized"
    ...
    async def async_press(self) -> None:
        ...
        if isinstance(result, dict):
            ...
            if isinstance(grade, int):
                derived_grade = grade
        self.hass.bus.async_fire(
            "qalaam_portion_marked_memorized",
            {
                "user_id": c.user_id,
...
async def _fetch_first_session_verse(c: QalaamCoordinator) -> str | None:
    ...
    try:
        async with c._session.get(
            ...
            if resp.status >= 400:  # noqa: PLR2004
                _LOGGER.debug("qalaam.test_me.session: HTTP %s", resp.status)
                return None
            body = await resp.json()
    except Exception as err:
        _LOGGER.debug("qalaam.test_me.session: %s", err)
        return None
...
async def _post_rate_current(c: QalaamCoordinator, body: dict[str, Any]) -> dict[str, Any] | None:
    """POST /v1/hifdh/rate-current. Returns the parsed response or None
    on failure (API key missing, network down, non-2xx)."""
    if not c.api_key:
        _LOGGER.debug("qalaam.mark_memorized: no api_key configured")
        return None
    try:
        async with c._session.post(
            ...
            if resp.status >= 400:  # noqa: PLR2004
                _LOGGER.warning("qalaam.mark_memorized: backend returned %s", resp.status)
                return None
            return await resp.json()
    except Exception as err:
        _LOGGER.warning("qalaam.mark_memorized: %s", err)
        return None


class QalaamRecordMistakeButton(QalaamEntity, ButtonEntity):
    ...
    def __init__(self, coordinator: QalaamCoordinator) -> None:
        ...
        self._attr_suggested_object_id = "qalaam_record_mistake_here"
    ...
    async def async_press(self) -> None:
        ...
        if not verse_key:
            _LOGGER.debug("qalaam.record_mistake: nothing playing")
            return
        if not c.api_key:
            _LOGGER.debug("qalaam.record_mistake: no api_key")
            return
        try:
            async with c._session.post(
                ...
                if resp.status >= 400:  # noqa: PLR2004
                    _LOGGER.warning(
                        "qalaam.record_mistake: backend returned %s on %s",
                        resp.status,
                        verse_key,
                    )
                    return
        except Exception as err:
            _LOGGER.warning("qalaam.record_mistake: %s", err)
            return
...
class QalaamReplayLastButton(QalaamEntity, ButtonEntity):
    ...
    def __init__(self, coordinator: QalaamCoordinator) -> None:
        ...
        self._attr_suggested_object_id = "qalaam_replay_last_portion"
    ...
    async def async_press(self) -> None:
        ...
        if not target or not verse_key:
            _LOGGER.debug(
                "qalaam.replay_last: missing target=%s or verse=%s", target, verse_key
            )
            return
        reciter = (
        ...
        try:
            ...
            _LOGGER.warning("qalaam.replay_last: %s", err)
