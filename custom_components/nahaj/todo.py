"""Nahaj todo entity — today's Hifdh plan as a TodoListEntity.
...
async def async_setup_entry(
    ...
    async_add_entities([NahajHifdhTodoList(coordinator)])


class NahajHifdhTodoList(NahajEntity, TodoListEntity):
    """Todo list backed by the Nahaj Hifdh session."""
    ...
    async def async_update_todo_item(self, item: TodoItem) -> None:
        for i, existing in enumerate(self._items):
            if existing.uid == item.uid:
                ...
                return
        _LOGGER.warning("nahaj.todo: unknown item uid %s", item.uid)
