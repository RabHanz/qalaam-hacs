...
"""DataUpdateCoordinator for Nahaj.
...
class NowPlayingSnapshot:
    """Backend's view of the nahaj virtual speaker — drives is_reciting + current_verse."""

    speaker_id: str = _NAHAJ_SPEAKER_ID
...
class NahajCoordinator(DataUpdateCoordinator[NahajSnapshot]):
    """Polls the Nahaj backend for catalog + Hifdh state + now-playing."""
    ...
    async def _async_update_data(self) -> NahajSnapshot:
    ...
    async def _fetch_catalog(self) -> dict[str, Any]:
        try:
            async with self._session.get(
                ...
                if resp.status == 401:
                    raise ConfigEntryAuthFailed("API key rejected by Nahaj backend.")
                if resp.status == 429:
                    raise UpdateFailed("Rate limited by Nahaj backend.")
    ...
    async def _fetch_hifdh_state(self, *, soft: bool) -> HifdhSnapshot:
        ...
        try:
            ...
            if soft:
                _LOGGER.debug("nahaj.hifdh_state.soft_fail: %s", err)
    ...
    async def _fetch_now_playing(self, *, soft: bool) -> NowPlayingSnapshot:
        try:
            async with self._session.get(
                f"{self.base_url}/v1/now-playing/{_NAHAJ_SPEAKER_ID}",
                headers=self._headers(),
                timeout=5,
            ) as resp:
                if resp.status >= 400:
            ...
            if soft:
                _LOGGER.debug("nahaj.now_playing.soft_fail: %s", err)
                return NowPlayingSnapshot()
            raise UpdateFailed(f"Now-playing failed: {err}") from err
        return NowPlayingSnapshot(
            speaker_id=str(p.get("speaker_id") or _NAHAJ_SPEAKER_ID),
    ...
    async def _fetch_topic_of_day(self, *, soft: bool) -> TopicOfDay:
        ...
        so each Nahaj home sees a different "topic of the day" but it
        ...
        try:
            ...
            if soft:
                _LOGGER.debug("nahaj.topic_of_day.soft_fail: %s", err)
                return TopicOfDay()
        ...
        try:
            ...
            _LOGGER.debug("nahaj.topic_of_day.sample_fetch_failed: %s", exc)
        return TopicOfDay(
            slug=str(t.get("slug", "")),
    ...
    async def _fetch_word_of_day(self, *, soft: bool) -> WordOfDay:
        ...
        try:
            ...
            if soft:
                _LOGGER.debug("nahaj.word_of_day.soft_fail: %s", err)
                return WordOfDay()
    ...
    async def _fetch_hijri(self, *, soft: bool) -> HijriToday:
        try:
            ...
            if soft:
                _LOGGER.debug("nahaj.hijri.soft_fail: %s", err)
                return HijriToday()
    ...
    async def _fetch_deep(self, *, soft: bool) -> HifdhDeepSnapshot:  # noqa: PLR0912 -- two soft-fail branches for two endpoints; splitting helpers adds boilerplate without clarity
        ...
        try:
            ...
            _LOGGER.debug("nahaj.deep.portions.soft_fail: %s", err)
        ...
        try:
            ...
            _LOGGER.debug("nahaj.deep.mistakes.soft_fail: %s", err)
    ...
    async def _fetch_prayer_window(self, *, soft: bool) -> PrayerWindow:
        ...
        if lat is None or lon is None:
            _LOGGER.debug("nahaj.prayer_window.no_location")
            return PrayerWindow()
    ...
    async def _fetch_prayer_times_for(
        ...
        try:
            ...
            if soft:
                _LOGGER.debug("nahaj.prayer_window.soft_fail %s: %s", date_iso, err)
                return {}
