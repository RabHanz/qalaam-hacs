"""Nahaj binary_sensor entities — backed by the coordinator's snapshots.
...
- ``binary_sensor.nahaj_in_prayer_window``   — current moment is inside a
...
- ``binary_sensor.nahaj_ramadan``            — current Hijri month is
  Ramadan. Drives the Ramadan-scenes blueprint + UI mode.
- ``binary_sensor.nahaj_last_ten_nights``    — last ten nights of Ramadan;
  triggers Laylat al-Qadr awareness scenes.
- ``binary_sensor.nahaj_friday_kahf_window`` — Thursday Maghrib through
...
automations decide what to do with them. Nahaj never silently changes
...
async def async_setup_entry(
    ...
    async_add_entities([NahajBinarySensor(coordinator, d) for d in _DESCRIPTIONS])


class NahajBinarySensor(NahajEntity, BinarySensorEntity):
    def __init__(
        self,
        coordinator: NahajCoordinator,
