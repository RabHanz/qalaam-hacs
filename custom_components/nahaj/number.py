"""Nahaj number entities — daily-pages quota for the active plan.

`number.nahaj_daily_pages_quota` mirrors the user's first ACTIVE
...
async def async_setup_entry(
    ...
    async_add_entities([NahajDailyQuotaNumber(coordinator)])


class NahajDailyQuotaNumber(NahajEntity, NumberEntity):
    ...
    def __init__(self, coordinator: NahajCoordinator) -> None:
        ...
        self._attr_suggested_object_id = "nahaj_daily_pages_quota"
    ...
    async def _refresh_plan(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug("nahaj.daily_quota.refresh: %s", err)
            return
    ...
    async def async_set_native_value(self, value: float) -> None:
        c = self.coordinator
        if not c.api_key:
            _LOGGER.warning("nahaj.daily_quota.write: no api_key configured")
            return
        if self._cached_plan_id is None:
            _LOGGER.warning(
                "nahaj.daily_quota.write: no active plan to write to — "
                "create a plan in the Nahaj web app first."
            )
            return
        ...
        try:
            async with c._session.patch(  # noqa: SLF001
                ...
                if resp.status >= 400:  # noqa: PLR2004
                    _LOGGER.warning(
                        "nahaj.daily_quota.write: PATCH /v1/plans/%s returned %s",
                        self._cached_plan_id,
                        resp.status,
                    )
                    return
        except Exception as err:  # noqa: BLE001
            _LOGGER.warning("nahaj.daily_quota.write: %s", err)
            return
