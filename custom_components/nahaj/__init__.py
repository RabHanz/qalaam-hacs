"""Nahaj Home Assistant integration entry point.

Per ADR-0003: thin shim over the standalone Nahaj backend. The HA integration
is one of many device adapters; it does NOT own the Nahaj state model.
...
from .panel import async_register_panel, async_remove_nahaj_panel
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Nahaj from a config entry."""
    # Pre-platform: rename any colliding generic entity_ids
    # (`sensor.nahaj`, `binary_sensor.nahaj_2`, …) left over from the
    ...
    coordinator = NahajCoordinator(hass, entry)
    try:
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok:
        ...
        # Only remove the panel if no other Nahaj entries remain.
        if not hass.config_entries.async_entries(DOMAIN):
            await async_remove_nahaj_panel(hass)
    return unload_ok
...
def _migrate_legacy_entity_ids(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Rename generic ``sensor.nahaj`` / ``binary_sensor.nahaj_<N>`` to
    canonical keyed slugs (``sensor.nahaj_ramadan_phase`` etc.).
