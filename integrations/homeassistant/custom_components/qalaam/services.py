...
- qalaam.pause_for_adhan    → DND: pause matching media_players for the
                              prayer window (or until manually resumed)
- qalaam.start_room_sabaq   → play current sabqi on every media_player
...
_PAUSE_FOR_ADHAN_SCHEMA = vol.Schema(
    {
        ...
        # pause every media_player.qalaam-prefixed entity AND any extra
        # entities the user has tagged with `qalaam_adhan_dnd: true`
        # in their customize.yaml — so a setup with multiple Qalaam
        ...
        # call this from an automation triggered by the qalaam_next_prayer
...
async def async_setup_services(hass: HomeAssistant) -> None:  # noqa: PLR0915 -- 5 service handlers naturally inline as nested closures; extracting them only adds boilerplate without clarity gain
    ...
    async def play_ayah(call: ServiceCall) -> None:
        ...
        _LOGGER.info(
            "qalaam.play_ayah: %s on %s (reciter=%s)", verse_key, call.data["target"], reciter
        )
    ...
    async def play_surah(call: ServiceCall) -> None:
        ...
        _LOGGER.info(
            "qalaam.play_surah: %s on %s (reciter=%s)",
            call.data["surah"],
            call.data["target"],
            reciter,
        )
    ...
    async def pause_for_adhan(call: ServiceCall) -> None:
        ...
        for entity_id in unique_targets:
            try:
                ...
                _LOGGER.warning("qalaam.pause_for_adhan: failed to pause %s", entity_id)
        duration = int(call.data.get("duration_minutes") or _DEFAULT_ADHAN_PAUSE_MIN)
        _LOGGER.info(
            "qalaam.pause_for_adhan: paused %d entities for %d min adhan window",
            len(unique_targets),
            duration,
        )
        # Fire an event so automations can chain (e.g. resume after
        # delay, send a notification "Qalaam paused for Maghrib").
        hass.bus.async_fire(
            "qalaam_adhan_pause_started",
    ...
    async def start_room_sabaq(call: ServiceCall) -> None:
        ...
        if not area:
            _LOGGER.warning("qalaam.start_room_sabaq: area %r not found", area_query)
            return
        ...
        if not media_targets:
            _LOGGER.warning(
                "qalaam.start_room_sabaq: no media_players in area %s (%s)",
                area.name,
                area.id,
            )
            return
        ...
        try:
            ...
            _LOGGER.warning(
                "qalaam.start_room_sabaq: bad portion_start %r — defaulting to 1:1",
                portion_start,
            )
            surah, ayah = 1, 1
        ...
        for entity_id in media_targets:
            try:
                ...
                _LOGGER.warning("qalaam.start_room_sabaq: play_media failed on %s", entity_id)
        _LOGGER.info(
            "qalaam.start_room_sabaq: %s playing %d:%d on %d entities in %s (reciter=%s)",
            reciter,
            surah,
            ayah,
            len(media_targets),
