"""Cross-device playback session bridge — Qalaam ⇄ Home Assistant.

Per ADR-0025 Phase 3: HA registers itself as a peer device in the user's
Qalaam playback session and reacts to load/play/pause/seek/transfer
...
push the new state back to the Qalaam session so all of the user's
other devices (phone, laptop, TV cast) stay in sync.

Wire model:

  Qalaam backend (SSE: /v1/playback/subscribe)
        │
...
`QalaamMediaPlayer._on_target_state_change` listener; here we add a
...
# Qalaam backend's heartbeat keeps the connection alive at 25 s.
...
class PlaybackBridge:
    ...
    async def _heartbeat_loop(self) -> None:
        """Register this HA instance as a device with the Qalaam
        ...
        try:
            async with self._session.post(
                ...
                if resp.status == _HTTP_UNAUTHORIZED:
                    _LOGGER.warning(
                        "Qalaam playback bridge: API key rejected; "
    ...
    async def _sse_once(self) -> None:
        ...
        dispatching `state` events to `_apply_state`. The Qalaam server
        emits `event: state\\ndata: {...}\\n\\n` per spec, plus
        ...
        async with self._session.get(
            ...
            if resp.status == _HTTP_UNAUTHORIZED:
                _LOGGER.warning("Qalaam SSE rejected — API key revoked or tier downgraded")
                self._stop.set()
                return
    ...
    async def _apply_state(self, state: dict[str, Any]) -> None:
        """Honour a state push from Qalaam by invoking the appropriate
        ...
        if media_changed and verse_key and reciter:
            # play_media — load new audio on the target. The Qalaam web
    ...
    # ─── push HA-originated state changes back to Qalaam ────────────
