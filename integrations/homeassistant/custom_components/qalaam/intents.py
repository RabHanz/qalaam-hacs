...
- QalaamExplainAyah   — "What does Surah 2 verse 255 mean?"
- QalaamFindVerse     — "Find a verse about patience"
- QalaamWordRoot      — "What's the root of istaghfir?"
"""
    ...
    try:
        async with session.post(
            ...
            if resp.status >= 400:  # noqa: PLR2004 -- HTTP error class
                _LOGGER.warning("qalaam mcp call %s -> %d", tool_name, resp.status)
                return None
            return await resp.json()
    except (TimeoutError, OSError) as err:
        _LOGGER.warning("qalaam mcp call %s failed: %s", tool_name, err)
        return None
